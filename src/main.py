...
from database import get_db_async, init_db, pwd_context, DB_PATH
from scraper import (
    start_scheduler,
    get_settings,
    MEDIA_DIR,
...
async def dashboard(request: Request, user: Optional[dict] = Depends(get_current_user)):
    ...
    if not request.cookies.get("csrf_token"):
        ...
        response.set_cookie(
            key="csrf_token", value=token, httponly=True, samesite="lax", path="/"
        )
    return response
...
async def get_wechat_status(user: dict = Depends(require_user)):
    """Checks if the browser profile session is valid (shared by WeChat and Sogou)."""
    profile_valid, profile_msg = await check_session_validity()
    sogou_msg = (
        "Sogou session active (shared profile)."
        if profile_valid
        else "Browser profile missing. Run auth.py."
    )

    return {
        "wechat": {"is_valid": profile_valid, "message": profile_msg},
        "sogou": {"is_valid": profile_valid, "message": sogou_msg},
    }
