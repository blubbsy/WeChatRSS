...
async def _process_account(page, conn, account_name, account_id):
    await add_system_log("INFO", "Scraper", f"Targeting: {account_name}")
    ...
    for url in urls[:10]:
        try:
            ...
            if not data:
                await add_system_log("DEBUG", "Fetch", f"Skipping {real_url}: {err}")
                continue
    ...
    await conn.execute(
        ...
        (datetime.datetime.now(), account_id, account_id),
    )
    await add_system_log(
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for account in accounts:
                await _process_account(page, conn, account["name"], account["id"])
