...
from database import get_db_async, DB_PATH
...
async def add_system_log(level, module, message):
    try:
        ...
        logger.warning(
            "Failed to write system log [%s/%s]: %s (error: %s)",
            level,
            module,
            message,
            e,
    ...
    try:
        ...
        if "去验证" in content or "环境异常" in content or "antispider" in content:
            await add_system_log(
                "WARNING", "Fetch", "Directly blocked. Using WeRead Proxy."
            )
            # 2. Fallback to Proxy
...
async def extract_account_info(url):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            logger.warning("Account info extraction failed: %s", e)
        await browser.close()
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for db_acc in db_accounts:
                ...
                try:
                    await page.goto(
                        f"https://weixin.sogou.com/weixin?type=2&query={name}",
                    ...
                    logger.warning("Sogou discovery failed for '%s': %s", name, e)
                    urls = []
                        ...
                        try:
                            ...
                            if data:
                                ...
                                async with conn.execute(
                                    ...
                                    (data["title"], target_biz),
                                ) as cur:
                                    if await cur.fetchone():
                                        # Update URL if it's the same title but a newer link
                                        await conn.execute(
                                            "UPDATE articles SET url = ? WHERE title = ? AND account_id = ?",
                                            (real_url, data["title"], target_biz),
                                        )
                                        continue

                                art_id = str(uuid.uuid4())
                                await conn.execute(
                                    """INSERT INTO articles (id, account_id, title, url, content, pub_date, fetch_date)
                                                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                                    (
                                        art_id,
                                        target_biz,
                            ...
                            logger.warning(
                                "Failed to process URL for '%s': %s", name, e
                            )
                            continue

                    await conn.execute(
                        "UPDATE accounts SET last_sync = ?, last_status = 'success', article_count = (SELECT COUNT(*) FROM articles WHERE account_id = ?) WHERE id = ?",
                        (datetime.datetime.now(), target_biz, target_biz),
                    )
                    await add_system_log(
                        "INFO", "Scraper", f"Synced '{name}' (+{new_count} articles)."
                    )
                    await conn.commit()
                else:
                    await add_system_log(
                        "WARNING",
                        "Scraper",
                        f"[{name}] No articles found on discovery.",
                    )
...
def start_scheduler():
    ...
    scheduler.add_job(
        scrape_wechat,
        "interval",
        hours=int(settings.get("fetch_interval_hours", 6)),
        jitter=int(settings.get("fetch_random_jitter_minutes", 30)) * 60,
        id="wechat_scraper",
        max_instances=1,
        coalesce=True,
    )
